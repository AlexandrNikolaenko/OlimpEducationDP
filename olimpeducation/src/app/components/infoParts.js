        ...
        return (
            <div className="pt-[78px] flex justify-between items-end gap-x-5">
                ...
                <div className="flex flex-col gap-y-3.5 items-end">
                    <Link href={`https://${host}/`} className="bg-bright font-sans text-sm/[18px] tracking-[0.02em] px-5  max-[1146px]:px-3.5 py-[5px] max-[1146px]:py-[4px] border-bright border-2 rounded-[10px]">Закрыть</Link>
    ...
    if (window.innerWidth > 700) {
        return (
            <div className="flex justify-between gap-x-5">
                <Link href={`https://${host}/`} className="bg-bright font-sans text-2xl/[25px] tracking-[0.02em] px-5 py-[5px] border-bright border-2 rounded-[10px]">Закрыть</Link>
                <DownloadTaskButton name={`${params.taskid[0]}.jpg`} url={url}/>
